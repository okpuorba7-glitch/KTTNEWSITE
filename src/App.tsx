...
const DEFAULT_SERVICES: Service[] = [
  { id:"food", cardClass:"food", label:"FOOD DELIVERY", emoji:"🍽️", number:"01",
    ...
    basePrice:"", deliveryFee:"", minOrder:"",
...
const DEFAULT_SETTINGS: Settings = {
  phone: "08160880608",
  email: "Kingstreattechltd@gmail.com",
...
export default function App() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // Do NOT auto-seed here to avoid permission errors for non-admins
        setServices(svcs.length > 0 ? svcs : DEFAULT_SERVICES);
        setPlans(plns.length > 0 ? plns : DEFAULT_PLANS);
        setSettings(stgs ? stgs : DEFAULT_SETTINGS);

      } catch (error) {
        console.error("Error loading data from Firestore (expected if unauthenticated and rules are strict)", error);
        // On error (e.g. initial read failure if rules were tighter), fallback to defaults
    ...
    const unsub = onAuthStateChanged(auth, (user) => {
      if (user) {
        // In a real app, we might check a custom claim or a document
        // For our rules, we've hardcoded the owner email as back-up admin
        const isOwner = user.email === "okpuorba7@gmail.com";
        setIsAdmin(isOwner); 
      } else {
        setIsAdmin(false);
      }
      ...
      <div className="admin-view">
        <nav className="nav" style={{top:settings.banner?"36px":0}}>
          ...
          <div className="nav-right">
            <span style={{background:"rgba(57,255,20,.1)",border:"1px solid rgba(57,255,20,.3)",color:"#39FF14",fontSize:11,fontWeight:700,padding:"4px 11px",borderRadius:20}}>🛡️ ADMIN MODE</span>
  ...
  return (
    <>
      {showLogin && <AdminLogin onLogin={handleAdminLogin} close={() => setShowLogin(false)} />}
